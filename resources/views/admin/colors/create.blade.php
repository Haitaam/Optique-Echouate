...
@section('content')
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 class="text-2xl font-bold text-white">Ajouter une Couleur</h1>
                <p class="text-heroi-text-muted text-sm mt-0.5">Nouvelle couleur de lunettes</p>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.colors.store') }}" class="space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="hex_value" value="{{ old('hex_value') }}" placeholder="#FF5733"
                        class="w-full rounded-xl bg-white/5 border border-white/10 text-white px-4 py-2.5 text-sm placeholder-heroi-text-muted/50 focus:outline-none focus:ring-2 focus:ring-orange-500/40">
                </div>
            </div>
